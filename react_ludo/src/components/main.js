...
class Main extends React.Component {
	...
	dice_btn_click(event) {
		...
		setTimeout(function(){$("#dice_value").html(num);},500);
		setTimeout(function(){this.move(num);}.bind(this),1000);
	}
	...
	move(num) {
		...
		if (pawns[color][index] == -1 && num != 6) {
			...
			previous_index=pawns[color][index];
		...
		if(previous_index!=-1){
			for(let i=previous_index+1;i<=pawns[color][index];i++){
				setTimeout(function(){
					$("#" + color + "-" + pawn_no).css("top", $('[data-' + color + '="' + i + '"]').css("top"));
					$("#" + color + "-" + pawn_no).css("left", $('[data-' + color + '="' + i + '"]').css("left"));
		...
		return (
			...
			<div >
				...
				<div class="cells dark-green pawn" style={{top: "40%", left:"13.32%"}}>2</div>
					...
					<br /><br />
					<div id="github" style={{ margin: "auto", width: "100px", visibility: "hidden" }}>
